...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^ {
        ...
        SEL newSelector = @selector(zcmurl_p_URLWithString:);
...
// 参数编码
+ (instancetype)zcmurl_p_URLWithString:(NSString *)URLString {
    NSString *newURLString = [URLString stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
    NSURL *url = [NSURL zcmurl_p_URLWithString:newURLString];
    if (!url) {
        ...
        if (markArray.count == 2) {
            ...
            return [NSURL zcmurl_p_URLWithString:newURLString];
        }
    }
    return url;
}

                                            ...
                                            data:(NSDictionary *)data
{
    ...
    id<LYResponseProtocol> response = [cls yy_modelWithDictionary:data[@"result"]];
    if (!response) {
        response = [cls new];
    }
    if ([response isKindOfClass:[LYResponseObject class]]) {
        response.resultcode = data[@"resultcode"];
        response.resultmsg = data[@"resultmsg"];
        response.resultDict = data[@"result"]; // 原始数据
    }
    return response;
}
